 ...
 *
 */
public class PictureLoadSecond {
	private Context mContext;

	private static PictureLoadSecond instance;
	/**
	 ...
	 *            线程调度方式
	 */
	private PictureLoadSecond(int threadCount, Type type) {
	 ...
	 * @return
	 */
	public static PictureLoadSecond getInstance(Context mContext) {
		if (instance == null) {
			synchronized (PictureLoadSecond.class) {
				if (instance == null) {
					instance = new PictureLoadSecond(DEFAULT_THREAD_COUNT, Type.FIFO);
	...
	public static PictureLoadSecond getInstances(int threadCount, Type lifo) {
		if (instance == null) {
			synchronized (PictureLoadSecond.class) {
				if (instance == null) {
					instance = new PictureLoadSecond(threadCount, lifo);
				}
			}
		}
		return instance;
	 ...
	 */
	@SuppressLint("HandlerLeak")
	public void loadImage(final String path, final CircleImageView imageView, final int pathId) {
		...
		if (bm != null) {
			...
			addTasks(new Runnable() {
				...
				public void run() {
					...
					Bitmap bm = decodeSampleBitmapFromPath(imageSize.width, imageSize.height, pathId);
					// 3.把图片加入到缓存
	 ...
	 * @return
	 */
	protected Bitmap decodeSampleBitmapFromPath(int width, int height, int path) {
		...
		BitmapFactory.decodeResource(mContext.getResources(), R.drawable.a10_1, options);
		...
		Bitmap bitmap = BitmapFactory.decodeResource(mContext.getResources(), path, options);
